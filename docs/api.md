...
每路在 episode 激活、基准就绪且无待确认候选时，将新解码的 BGR 帧放入最多 6 帧的
独立队列，并通知共享推理线程。调度器在有帧且未超限的摄像头间按最近服务顺序轮转，
每次处理一帧；同一路两次调度开始的间隔至少为 1 / inference_fps，推理耗时计入间隔。
限频时间记录在调用该路处理逻辑之前；锁等待等开销会影响实际进入模型的时刻。
没有新帧时不会消耗该路的推理机会，也不会重复处理同一帧。空闲时仍执行超时维护。
ring_max_fps 只控制历史 JPEG 缓存采样上限，推理直接使用解码像素。
默认目标为 8 FPS；实际吞吐取决于源帧率和多路总负载，不能由配置值保证。
...
- queue_depth：当前待推理帧数，范围为 0 到 6，不含正在推理的帧。
- frames_dropped：已入队但未送入模型的累计丢弃帧数，包括队列满、帧过期、断流，
  以及基准、会话或检测配置变更时清空的帧；不计暂停期间未入队的帧，也不计网络丢包。
...
队列按接收顺序消费；满时丢最旧帧，并在消费前丢弃超过 frame_max_age_seconds 的帧。
暂停检测、更新基准或检测配置、结束会话与断流都会清空待处理帧，恢复后等待新帧。
队列容量限制积压帧数，但不是端到端延迟保证，正在执行的模型推理不会被中断。
目标 FPS 相同不代表
...
已成功回执的候选在记录保留期内可以安全重试，即使摄像头已删除、禁用，或服务已经
重启；相同 episode_id 和 triggered_vlm 返回已保存的结果，baseline 不再解析或应用。
冲突的处理结果返回 409。并发编码期间若会话
